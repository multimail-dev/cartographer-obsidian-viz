 ...
 * Does NOT:  require Cloudflare, D1, R2, KV, or any cloud account. Zero network
 *            dependencies beyond loading Google Fonts in the browser.
 * Use instead of:  `wrangler dev` when you want a pure-local experience.
 *
 * Usage:
...
// Static asset roots (built by `bun run build` in apps/atlas-ui)
const WORKER_PUBLIC = join(import.meta.dir, "..", "..", "workers", "vault-mcp", "public");
...
// ---------------------------------------------------------------------------
// HTML — same as the worker's index.html but WITHOUT the power-mode auto-inject
// ---------------------------------------------------------------------------

function localIndexHtml(): string {
  // Read the worker's index.html and strip the power-mode bootstrap script
  // that forces ?power=<origin>. In local mode the UI fetches /api/graph
  // (single payload) from the same origin — no pagination, no bearer auth.
  const workerHtml = Bun.file(join(WORKER_PUBLIC, "index.html"));
  // We'll do this synchronously at startup — see below
  return "";
}
...
async function loadIndexHtml(): Promise<void> {
  const raw = await Bun.file(join(WORKER_PUBLIC, "index.html")).text();
  // Strip the power-mode bootstrap <script> block
  cachedIndexHtml = raw.replace(
    /\s*<script>\s*\/\/ Power-mode bootstrap[\s\S]*?<\/script>/,
    "",
  );
}
  ...
  Bun.serve({
    ...
    async fetch(req) {
      ...
      if (url.pathname === "/style.css") {
        const file = Bun.file(join(WORKER_PUBLIC, "style.css"));
      ...
      if (url.pathname === "/dist/app.js") {
        const file = Bun.file(join(WORKER_PUBLIC, "dist", "app.js"));
        return new Response(file, {
      ...
      if (url.pathname === "/dist/fa3-worker.js") {
        const file = Bun.file(join(WORKER_PUBLIC, "dist", "fa3-worker.js"));
        return new Response(file, {
          headers: { "Content-Type": "application/javascript; charset=utf-8" },
        });
      }
