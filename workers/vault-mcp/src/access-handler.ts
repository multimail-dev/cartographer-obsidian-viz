...
/**
 * Minimal Env shape required by the access handler.
 */
...
type EnvWithOauth = OAuthHandlerEnv & { OAUTH_PROVIDER: OAuthHelpers };

/**
 * Factory that returns a fetch handler for OAuth/CF Access flows.
 * Pass server branding as arguments; the returned function is the fetch handler.
 *
 * @param serverName        Display name shown on the OAuth approval dialog (e.g. "Vault MCP")
 * @param serverDescription One-line description shown on the OAuth approval dialog
 */
export function createAccessHandler(serverName: string, serverDescription: string) {
	return async function handleAccessRequest(
		request: Request,
		env: EnvWithOauth,
...
/**
 * Default vault-mcp access handler — pre-bound with vault-mcp branding.
 * Import and use this directly in vault-mcp's index.ts.
 */
export const handleAccessRequest = createAccessHandler(
	"Vault MCP",
	"Obsidian vault graph — search, write, and explore notes via MCP.",
);
 ...
 */
async function fetchAccessPublicKey(env: OAuthHandlerEnv, kid: string) {
 ...
 */
async function verifyToken(env: OAuthHandlerEnv, token: string) {
